...
from Syphix import userbot
from Syphix.core.mongo import mongodb
...
async def set_assistant(chat_id):
    from Syphix.core.userbot import assistants
...
async def is_on_off(chat_id: int) -> bool:
    """
    Placeholder function to check if a feature is enabled for the chat.
    Always returns True for now.
    """
    return True


async def get_spam_data(user_id: int) -> int:
    """
    Return count of commands used by user in the last 60 seconds.
    """
    now = datetime.utcnow()
    user_times = SPAM_DATA.get(user_id, [])
    # Keep only recent timestamps (within 60 seconds)
    recent = [t for t in user_times if now - t < timedelta(seconds=60)]
    SPAM_DATA[user_id] = recent
    return len(recent)


async def add_spam_data(user_id: int):
    """
    Register a command use for spam tracking.
    """
    now = datetime.utcnow()
    SPAM_DATA.setdefault(user_id, []).append(now)


async def block_user(user_id: int, seconds: int = 600):
    """
    Temporarily block a user for 'seconds' (default: 10 min).
    """
    unblock_time = datetime.utcnow() + timedelta(seconds=seconds)
...
async def is_user_blocked(user_id: int) -> bool:
    ...
    """
    expiry = BLOCKED_USERS.get(user_id)
